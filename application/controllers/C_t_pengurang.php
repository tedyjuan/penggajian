...
class c_t_pengurang extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_t_pengurang';
        $this->view = 'v_t_pengurang';
        /* mulai load  di folder model */
        $this->load->model('m_t_pengurang', 't_pengurang'); /* cara pemanggilan Mdepartment_model menjadi department */
        $this->load->model('m_pengurang', 'pengurang');/* selesai load Mdepartment_model.php di folder model */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->t_pengurang->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - t_pengurang'; //setting judul, yang akan berubah di form.php
       $resulttambah = $this->pengurang->getAll();
        $i = 0;
        foreach ($resulttambah as $rowtambah) {
            $data['default']['id_pengurang'][-1]['value'] = NULL;
            $data['default']['id_pengurang'][-1]['display'] = '- Please Select -';
            $data['default']['id_pengurang'][$i]['value'] = $rowtambah['id_pengurang'];
            $data['default']['id_pengurang'][$i]['display'] = $rowtambah['id_pengurang'] . ' - ' . $rowtambah['nama_pengurang'];
            $i++;
        } 
    ...
    public function addpost() {
        $this->form_validation->set_rules('id_pengurang', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
		...
		if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $id_pengurang = $this->input->post('id_pengurang'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type
            ...
            $checkdata = $this->t_pengurang->checkdata($id_karyawan,$id_pengurang,$periode_gaji); // melakukan pengecekan data
            if ($checkdata > 0) { /* jika data bernilai lebih dari 0, maka data tidak tersimpan, karena sudah ada */
                ...
                $this->t_pengurang->insert($record);
                $valid = 'true';
            ...
            $jsonmsg = array(
				...
				"err_id_pengurang" => form_error('id_pengurang'),
    ...
    function edit($id) {
        $row = $this->t_pengurang->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - t_pengurang';
		$resulttambah = $this->pengurang->getAll();
        ...
        foreach ($resulttambah as $rowtambah) {
            ...
            $data['default']['id_pengurang'][$i]['value'] = $rowtambah['id_pengurang'];
            $data['default']['id_pengurang'][$i]['display'] = $rowtambah['id_pengurang'] . ' - ' . $rowtambah['nama_pengurang'];
            if ($row->id_pengurang == $rowtambah['id_pengurang']) {
                $data['default']['id_pengurang'][$i]['selected'] = "SELECTED";
            }
			$i++;
        } 
    ...
    function editpost() {
		...
		$this->form_validation->set_rules('id_pengurang', '', 'required');
		$this->form_validation->set_rules('periode_gaji', '', 'required');
		$this->form_validation->set_rules('catatan', '', 'required');
	   ...
	   if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->t_pengurang->update($id, $record);
    ...
    public function remove() {
        ...
        $this->t_pengurang->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
