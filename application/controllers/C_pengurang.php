...
class c_pengurang extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_pengurang';
        $this->view = 'v_pengurang';
        /* mulai load  di folder model */
        $this->load->model('m_pengurang', 'pengurang'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->pengurang->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - pengurang'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_pengurang'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_pengurang', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $pengurang = $this->input->post('nama_pengurang'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type

            /* membuat record sebuah array, array ini akan masuk ke database */
            $record = array(
                "nama_pengurang" => $pengurang,
            );

            $checkdata = $this->pengurang->checkdata($pengurang); // melakukan pengecekan data
            if ($checkdata > 0) { /* jika data bernilai lebih dari 0, maka data tidak tersimpan, karena sudah ada */
                $valid = 'false';
                $message = 'data already exist';
                $err_name = "Name pengurang sudah ada";
            } else { /* jika data belum ada,maka berhasil di simpan */
                $this->pengurang->insert($record);
                $valid = 'true';
            ...
            $jsonmsg = array(
                ...
                "hasil" => 'false',
                "err_nama_pengurang" => form_error('nama_pengurang'),
            );
        }
    ...
    function edit($id) {
        $row = $this->pengurang->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - pengurang';
        $data['default']['nama_pengurang'] = $row->nama_pengurang; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_pengurang', 'nama_pengurang', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->pengurang->update($id, $record);

            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
                "msg" => 'Update Data Success',
                "hasil" => 'true',
                "err_nama_pengurang" => null,
            );
        } else {
            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
    ...
    public function remove() {
        ...
        $this->pengurang->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
