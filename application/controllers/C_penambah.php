...
class c_penambah extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_penambah';
        $this->view = 'v_penambah';
        /* mulai load  di folder model */
        $this->load->model('m_penambah', 'penambah'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->penambah->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - penambah'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_penambah'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_penambah', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $penambah = $this->input->post('nama_penambah'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type

            /* membuat record sebuah array, array ini akan masuk ke database */
            $record = array(
                "nama_penambah" => $penambah,
            );

            $checkdata = $this->penambah->checkdata($penambah); // melakukan pengecekan data
            if ($checkdata > 0) { /* jika data bernilai lebih dari 0, maka data tidak tersimpan, karena sudah ada */
                $valid = 'false';
                $message = 'data already exist';
                $err_name = "Name penambah sudah ada";
            } else { /* jika data belum ada,maka berhasil di simpan */
                $this->penambah->insert($record);
                $valid = 'true';
            ...
            $jsonmsg = array(
                ...
                "hasil" => 'false',
                "err_nama_penambah" => form_error('nama_penambah'),
            );
        }
    ...
    function edit($id) {
        $row = $this->penambah->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - penambah';
        $data['default']['nama_penambah'] = $row->nama_penambah; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_penambah', 'nama_penambah', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->penambah->update($id, $record);

            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
                "msg" => 'Update Data Success',
                "hasil" => 'true',
                "err_nama_penambah" => null,
            );
        } else {
            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
    ...
    public function remove() {
        ...
        $this->penambah->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
