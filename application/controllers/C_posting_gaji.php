...
class c_posting_gaji extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_posting_gaji';
        $this->view = 'v_posting_gaji';
        /* mulai load  di folder model */
        $this->load->model('m_posting_gaji', 'posting_gaji'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->posting_gaji->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - posting_gaji'; //setting judul, yang akan berubah di form.php
    ...
    public function addpost() {
		...
		if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $checkdata = $this->posting_gaji->checkdata($id_karyawan); // melakukan pengecekan data
            if ($checkdata > 0) { /* jika data bernilai lebih dari 0, maka data tidak tersimpan, karena sudah ada */
                ...
                $this->posting_gaji->insert($record);
                $valid = 'true';
    ...
    function edit($id) {
        $row = $this->posting_gaji->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - posting_gaji';
		$resulttambah = $this->penambah->getAll();
    ...
    function editpost() {
	   ...
	   if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->posting_gaji->update($id, $record);
    ...
    public function remove() {
        ...
        $this->posting_gaji->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
