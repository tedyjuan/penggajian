...
class c_statusnikah extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_statusnikah';
        $this->view = 'v_statusnikah';
        /* mulai load  di folder model */
        $this->load->model('m_statusnikah', 'statusnikah'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->statusnikah->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - Status Nikah'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_status'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_status', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $status = $this->input->post('nama_status'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type
            ...
            $jsonmsg = array(
                ...
                "hasil" => 'false',
                "err_nama_status" => form_error('nama_status'),
            );
        }
    ...
    function edit($id) {
        $row = $this->statusnikah->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - Status Nikah';
        $data['default']['nama_status'] = $row->nama_status; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_status', 'nama_status', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->statusnikah->update($id, $record);

            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
                "msg" => 'Update Data Success',
                "hasil" => 'true',
                "err_nama_status" => null,
            );
        } else {
            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
    ...
    public function remove() {
        ...
        $this->statusnikah->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
