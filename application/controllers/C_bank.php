...
class c_bank extends CI_Controller {
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->bank->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - bank'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_bank'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_bank', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $bank = $this->input->post('nama_bank'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type
    ...
    function edit($id) {
        $row = $this->bank->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - bank';
        $data['default']['nama_bank'] = $row->nama_bank; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_bank', 'nama_bank', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
    ...
    public function remove() {
        ...
        $this->bank->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
