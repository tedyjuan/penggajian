...
class c_t_lembur extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_t_lembur';
        $this->view = 'v_t_lembur';
        /* mulai load  di folder model */
        $this->load->model('m_t_lembur', 't_lembur'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->t_lembur->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - Transaksi Lembur'; //setting judul, yang akan berubah di form.php
		...
		$data['default']['nomor_spkl'] = ''; 
		$data['default']['tanggal_kehadiran'] = '';
		$data['default']['mulai_lembur'] = '';
		$data['default']['selesai_lembur'] = '';
    ...
    public function addpost() {
		...
		$this->form_validation->set_rules('nomor_spkl', 'nomor_spkl', 'required');
		$this->form_validation->set_rules('tanggal_kehadiran', 'tanggal_kehadiran', 'required');
		$this->form_validation->set_rules('mulai_lembur', 'mulai_lembur', 'required');
		$this->form_validation->set_rules('selesai_lembur', 'selesai_lembur', 'required');
		...
		if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
			...
			$nomor_spkl = $this->input->post('nomor_spkl');
			$tanggal_kehadiran = $this->input->post('tanggal_kehadiran');
            ...
            $record = array(
				...
				"nomor_spkl" => $nomor_spkl,	
				"tanggal_kehadiran" => date('Y-m-d',strtotime($tanggal_kehadiran)),
				"mulai_lembur" => date('Y-m-d H:i:s',strtotime($mulai_lembur)),
				"selesai_lembur" => date('Y-m-d H:i:s',strtotime($selesai_lembur)),
				"catatan" => $catatan,
				
				
            );

            $checkdata = $this->t_lembur->checkdata($id_karyawan,$nomor_spkl,$tanggal_kehadiran,$mulai_lembur); // melakukan pengecekan data
            ...
            $jsonmsg = array(
				...
				"err_nomor_spkl" => form_error('nomor_spkl'),
				"err_tanggal_kehadiran" => form_error('tanggal_kehadiran)'),
				"err_mulai_lembur" => form_error('mulai_lembur'),
				"err_selesai_lembur" => form_error('selesai_lembur'),
    ...
    function edit($id) {
        $row = $this->t_lembur->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
      //print_r($row);
	//exit;
	  $data['title'] = 'Edit - Transaksi Lembur';
		...
		$data['default']['nomor_spkl'] = $row->nomor_spkl; 
		$data['default']['tanggal_kehadiran'] =$row->tanggal_kehadiran;
		$data['default']['mulai_lembur'] = $row->mulai_lembur;
		$data['default']['selesai_lembur'] =$row->selesai_lembur;
    ...
    function editpost() {
	   ...
	   if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->t_lembur->update($id, $record);
    ...
    public function remove() {
        ...
        $this->t_lembur->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
