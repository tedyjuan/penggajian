...
class c_pendidikan extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_pendidikan';
        $this->view = 'v_pendidikan';
        /* mulai load  di folder model */
        $this->load->model('m_pendidikan', 'pendidikan'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->pendidikan->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - pendidikan'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_pendidikan'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_pendidikan', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $pendidikan = $this->input->post('nama_pendidikan'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type
            ...
            $jsonmsg = array(
                ...
                "hasil" => 'false',
                "err_nama_pendidikan" => form_error('nama_pendidikan'),
            );
        }
    ...
    function edit($id) {
        $row = $this->pendidikan->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - pendidikan';
        $data['default']['nama_pendidikan'] = $row->nama_pendidikan; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_pendidikan', 'nama_pendidikan', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->pendidikan->update($id, $record);

            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
                "msg" => 'Update Data Success',
                "hasil" => 'true',
                "err_nama_pendidikan" => null,
            );
        } else {
            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
    ...
    public function remove() {
        ...
        $this->pendidikan->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
