...
class c_jabatan extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->controller = 'c_jabatan';
        $this->view = 'v_jabatan';
        /* mulai load  di folder model */
        $this->load->model('m_jabatan', 'jabatan'); /* cara pemanggilan Mdepartment_model menjadi department */
    ...
    public function grid() {
        echo json_encode(array(
                "data" => $this->jabatan->getGridData()->result() /* mengakses ke model department, ke fungsi getGridData, lalu hasilnya sebuah array assosiatif */
    ...
    function add() {
        $data['title'] = 'Add - jabatan'; //setting judul, yang akan berubah di form.php
        $data['default']['nama_jabatan'] = ''; // setting input dengan type properties namenya type, defaultnya kosong
    ...
    public function addpost() {
        $this->form_validation->set_rules('nama_jabatan', '', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            $jabatan = $this->input->post('nama_jabatan'); // menangkap post dari form.php ketika add data, dengan properties namenya adalah type
            ...
            $jsonmsg = array(
                ...
                "hasil" => 'false',
                "err_nama_jabatan" => form_error('nama_jabatan'),
            );
        }
    ...
    function edit($id) {
        $row = $this->jabatan->getby_id($id)->row(); /* mendapatkan nilai data berdasarkan id, dan berupa row, yaitu 1 data */
        $data['title'] = 'Edit - jabatan';
        $data['default']['nama_jabatan'] = $row->nama_jabatan; /* setting isi properties type dengan datanya */
    ...
    function editpost() {
        $this->form_validation->set_rules('nama_jabatan', 'nama_jabatan', 'required'); //pengecekan, jika properties input type kosong, data tidak akan tersimpan,dan field tersebut harus diisi
        if ($this->form_validation->run() == TRUE) { // jika field yang dibutuhkan telah terisi maka nilai true
            ...
            $this->jabatan->update($id, $record);

            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
                "msg" => 'Update Data Success',
                "hasil" => 'true',
                "err_nama_jabatan" => null,
            );
        } else {
            /* membuat array, yang akan dikonversi menjadi json untuk kebutuhan ajax */
            $jsonmsg = array(
    ...
    public function remove() {
        ...
        $this->jabatan->delete($id); /* mengakses model department, lalu ke fungsi delete dengan parameter sebuah id */
