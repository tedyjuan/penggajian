...
class M_penambah extends CI_Model {
    ...
    public function __construct() {
        parent::__construct();
        $this->table = "m_penambah"; //setting di dalam $this->table dalah nama tabel sesuai di databasenya
        $this->prefix_id = "id_penambah"; //setting di dalam $this->prefix_id adalah primary key di tabel
    }

    /* mendapatkan semua data dan hasilnya sebuah array */

    public function getAll() {
        $this->db->order_by('nama_penambah','asc');
        return $this->db->get($this->table)->result_array();
    }

    function checkData($param) {
        $this->db->where("nama_penambah", $param);
